...
namespace NBShaderEditor
{
    internal static class NBShaderInspectorLocalization
    {
        ...
        private const string LanguagePreferenceKey = "NBShaderEditor.Localization.Language";
        ...
        public static GUIContent MakeContent(string labelKey, string labelFallback, string tooltipKey, string tooltipFallback)
        {
            return new GUIContent(
                Get(labelKey, labelFallback),
                Get(tooltipKey, tooltipFallback));
        }
        ...
        private static void EnsureLoaded()
        {
            ...
            if (csvAsset == null)
            {
                Debug.LogWarning($"NBShader inspector localization csv not found at '{CsvAssetPath}'.");
                return;
            }
        ...
        private static void ParseCsv(string csvText)
        {
            ...
            for (int rowIndex = 1; rowIndex < rows.Count; rowIndex++)
            {
                ...
                Dictionary<string, string> rowTable = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
                for (int columnIndex = 1; columnIndex < headers.Count; columnIndex++)
                {
                    string language = headers[columnIndex];
                    string value = columnIndex < values.Count ? values[columnIndex] : string.Empty;
                    rowTable[language] = value;
                }

                _table[values[0]] = rowTable;
            }
        }
        ...
        {
            EnsureLoaded();
            string preferred = EditorPrefs.GetString(LanguagePreferenceKey, GetLanguageFromEditor());
